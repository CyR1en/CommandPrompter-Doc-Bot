...
- **Discord-safe markdown sanitization** — before delivery, every answer is run through a sanitizer that rewrites Discord-incompatible markdown (GFM tables, horizontal rules, H4+ headers, raw HTML) into Discord-friendly equivalents and normalizes spacing. The agent persona also instructs the model to avoid these formats, but the sanitizer is a defense-in-depth so the user never sees broken rendering.
...
- **Hermetic test suite** — 280+ `pytest` tests with fakes for the LLM client, Git manager, and Discord client; no live API keys or network access needed.
...
│   ├── markdown_sanitizer.py    # sanitize_markdown: rewrites GFM tables, HRs, H4+ into Discord-safe markdown
│   ├── message_splitter.py      # split_message: content-aware chunking for embed descriptions
